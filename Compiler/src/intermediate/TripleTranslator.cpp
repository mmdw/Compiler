...
void TripleTranslator::translate(ASTBuilder::SymbolTable* p_table, TypeTable* p_type, std::ostream& os,
	...
	for (std::list<Triple>::iterator it = tripleSequence.begin(); it != tripleSequence.end(); ++it) {
		...
		switch(triple.op) {
		case TRIPLE_OR:
			append(os, mov(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1))));
			append(os, _or(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, mov(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.result)), EAX));
			break;
		case TRIPLE_AND:
			append(os, mov(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, _and(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, mov(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.result)), EAX));
			break;
		case TRIPLE_ADD_INT:
			append(os, mov(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, add(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, mov(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.result)), EAX));
			break;
		case TRIPLE_ADD_FLOAT:
			append(os, fld(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, fadd(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, fstp(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.result))));
			break;
		case TRIPLE_SUB_INT:
			append(os, mov(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, sub(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, mov(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.result)), EAX));
			break;
		case TRIPLE_SUB_FLOAT:
			append(os, fld(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, fsub(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, fstp(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.result))));
			break;
		case TRIPLE_MUL_INT:
			append(os, mov(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, imul(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, mov(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.result)), EAX));
			break;
		case TRIPLE_MUL_FLOAT:
			append(os, fld(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, fmul(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, fstp(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.result))));
			break;
		case TRIPLE_DIV_INT:
			append(os, mov(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, _xor(EDX, EDX));
			append(os, idiv(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, mov(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.result)), EAX));
			break;
		case TRIPLE_DIV_FLOAT:
			append(os, fld(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, fdiv(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, fstp(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.result))));
			break;
		...
		case TRIPLE_RETURN_FUNCTION:
			append(os, lea(ESI, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
		...
		case TRIPLE_PUSH_INT:
			append(os, push(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			break;
		case TRIPLE_CALL_FUNCTION:
			append(os, call(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1)));
			break;
		...
		case TRIPLE_PRINTLN_INT:
			append(os, printInt(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
		...
		case TRIPLE_PRINTLN_DOUBLE_FLOAT:
			append(os, printDoubleFloat(
					b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1)),
					b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1) + " + 4")
				));
		...
		case TRIPLE_READLN_INT: {
			append(os, lea(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1))));
			append(os, push(EAX));
		...
		case TRIPLE_INT_TO_FLOAT:
			append(os, fild(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1))));
			append(os, fstp(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.result))));
			break;
		case TRIPLE_FLOAT_TO_DOUBLE_FLOAT:
			append(os, fld_dword(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1))));
			append(os, fstp_qword(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.result))));
			break;
		...
		case TRIPLE_NEG_FLOAT:
			append(os, "fldz");
			append(os, fsub(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1))));
			append(os, fstp(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.result))));
			break;
		case TRIPLE_NOT:
			append(os, mov(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1))));
		...
		case TRIPLE_LESS_EQUAL_INT:
			...
			append(os, mov(EBX, "-1"));
			append(os, mov(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg1))));
			append(os, cmp(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type,triple.arg2))));
			append(os, inst(cmpInstruction(triple.op) + '\t', ECX, EBX));
			append(os, mov(b(CodeGenerator::symbolToAddr(p_table, p_type,triple.result)), ECX));
			break;
		...
		case TRIPLE_LESS_FLOAT:
			...
			append(os, fld(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg2))));
			append(os, fld(b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1))));
			...
			break;
		case TRIPLE_LABEL:
			append(os, CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1) + ':');
			break;
		case TRIPLE_JZ:
			append(os, mov(EAX, b(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1))));
			append(os, cmp(EAX, "0"));
			append(os, jz(CodeGenerator::symbolToAddr(p_table, p_type, triple.result)));
			break;
		case TRIPLE_JMP:
			append(os, jmp(CodeGenerator::symbolToAddr(p_table, p_type, triple.arg1)));
			break;
		default:
